...
export const software = pgTable("software", {
  ...
  logoUrl: text("logo_url"), // 软件Logo链接
...
export const softwareVersionHistory = pgTable("software_version_history", {
  ...
  // 版本标签和分类
  versionType: varchar("version_type", { length: 20 })
    .default("release")
    .notNull(), // release, beta, alpha, rc
...
export const downloadStats = pgTable("download_stats", {
  ...
  softwareId: integer("software_id")
    .notNull()
    .references(() => software.id, { onDelete: "cascade" }),
  versionId: integer("version_id").references(() => softwareVersionHistory.id, {
    onDelete: "cascade",
  }),
...
export type NewSoftwareVersionHistory =
  typeof softwareVersionHistory.$inferInsert;
